...
async function seedParts() {
  try {
    ...
    // Test with just one part first
    console.log('📝 Testing with first part...');
    const firstPart = parts[0];

    const testResult = await Part.create(firstPart);
    console.log(`✅ Successfully created test part: ${testResult.name}`);

    // If test works, proceed with all parts
    console.log(`📝 Inserting remaining ${parts.length - 1} parts...`);

    const remainingParts = parts.slice(1);
    const result = await Part.insertMany(remainingParts, { ordered: false });

    console.log('');
    console.log('📊 Seeding Summary:');
    console.log(`✅ Successfully created: ${result.length + 1} parts`);
    ...
    if (error.name === 'BulkWriteError') {
      console.log('');
      console.log('📊 Seeding Summary:');
      console.log(`✅ Successfully created: ${error.result.nInserted} parts`);
      console.log(`❌ Failed (duplicates): ${error.result.writeErrors.length} parts`);
      console.log(`📈 Total attempted: ${parts.length} parts`);

      // Category breakdown for successful inserts
      const successfulParts = error.result.insertedIds ? Object.keys(error.result.insertedIds).length : 0;
      console.log(`\n📋 Successfully inserted: ${successfulParts} parts`);
    } else {
